...
public class RecommendationsServiceImpl implements RecommendationsService {
    ...
    public Stream<RecommendedEventProto> getRecommendationsForUser(
        ...
        log.info("Generating recommendations for userId={}, maxResults={}",
                requestProto.getUserId(), requestProto.getMaxResults());

        List<UserAction> touchedEvents = userActionRepository.findEventsByUserIdOrderByTimestampDesc(
                requestProto.getUserId(), requestProto.getMaxResults());

        if (touchedEvents.isEmpty()) {
            log.debug("No user actions found for userId={}, returning empty recommendations",
                    requestProto.getUserId());
            return Stream.empty();
        }

        log.debug("Found {} recent user actions for userId={}", touchedEvents.size(), requestProto.getUserId());
        ...
        Map<Long, Double> eventScopes = touchedEvents.stream()
                .collect(Collectors.toMap(
                        UserAction::getEventId,
                        userAction -> convertActionTypeToWeight(userAction.getActionType()),
                        Math::max));

        log.debug("Calculated event scopes for {} events", eventScopes.size());

        // Find similar events based on touched events
        List<EventSimilarity> similarEvents = eventSimilarityRepository.findSimilarEvents(
        ...
        for (EventSimilarity eventSimilarity : similarEvents) {
            Long candidateEventId = touchedEventIds.contains(eventSimilarity.getEventA())
                    ? eventSimilarity.getEventB()
                    : eventSimilarity.getEventA();

            Long userEventId = touchedEventIds.contains(eventSimilarity.getEventA())
                    ? eventSimilarity.getEventA()
                    : eventSimilarity.getEventB();
            ...
            if (rate != null) {
                ...
                log.trace("Calculated score for candidate event {}: {} * {} = {}",
                        candidateEventId, rate, eventSimilarity.getScore(), score);
            }
        }

        // Sort by score descending and map to proto
        Stream<RecommendedEventProto> result = candidateScores.entrySet().stream()
        ...
        log.info("Generated {} recommendations for userId={}",
                candidateScores.size(), requestProto.getUserId());
    ...
    public Stream<RecommendedEventProto> getSimilarEvents(SimilarEventsRequestProto requestProto) {
        log.info("Finding similar events for eventId={}, userId={}, maxResults={}",
        ...
        if (eventsIds.isEmpty()) {
            log.debug("No events found for userId={}, returning empty similar events",
                    requestProto.getUserId());
            return Stream.empty();
        }
        ...
        log.debug("Found {} user events (excluding target) for similarity search", eventsIds.size());

        List<EventSimilarity> similarEvents = eventSimilarityRepository.findSimilarEventsByEventsIds(
                requestProto.getEventId(), eventsIds);

        log.debug("Found {} direct similarity records for eventId={}",
                similarEvents.size(), requestProto.getEventId());

        // Build map of similar event IDs with their scores
        Map<Long, Double> similarIdsWithScope = new HashMap<>();
        for (EventSimilarity eventSimilarity : similarEvents) {
            Long similarEventId = requestProto.getEventId() == eventSimilarity.getEventA()
                    ? eventSimilarity.getEventB()
                    : eventSimilarity.getEventA();
            similarIdsWithScope.put(similarEventId, eventSimilarity.getScore());
            log.trace("Added similar event: id={}, score={}", similarEventId, eventSimilarity.getScore());
        }

        // Sort by score descending and map to proto
        Stream<RecommendedEventProto> result = similarIdsWithScope.entrySet().stream()
                .sorted((e1, e2) -> Double.compare(e2.getValue(), e1.getValue()))
                .map(entry -> RecommendedEventProto.newBuilder()
                        .setEventId(entry.getKey())
                        .setScore(entry.getValue())
        ...
        log.info("Returning {} similar events for eventId={}",
                similarIdsWithScope.size(), requestProto.getEventId());
    ...
    public Stream<RecommendedEventProto> getUserActionsCount(
        ...
        List<UserAction> userActions = userActionRepository.findUserActionByEventIdIn(requestProto.getEventIdList());
        ...
        return requestProto.getEventIdList().stream()
                .map(eventId -> {
                    double weight = userActions.stream()
                            ...
                            .mapToDouble(userAction -> convertActionTypeToWeight(userAction.getActionType()))
                            .sum();

                    log.trace("Calculated total weight for eventId={}: {}", eventId, weight);
